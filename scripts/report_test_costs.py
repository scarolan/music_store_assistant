...
def format_report(stats: dict, markdown: bool = False) -> str:
    """Format stats as a human-readable report.

    Args:
        stats: Dictionary of token usage statistics
        markdown: If True, format for GitHub Step Summary (markdown tables)
    """
    if markdown:
        return _format_markdown(stats)
    return _format_terminal(stats)


def _format_terminal(stats: dict) -> str:
    """Format for terminal output with ASCII separators."""
    lines = [
        "",
        "=" * 65,
...
def _format_markdown(stats: dict) -> str:
    """Format for GitHub Step Summary with proper markdown tables."""
    lines = [
        "### 🔥 LangSmith Test Run Cost Report",
        "",
        f"**Time Window:** `{stats['start_time'][:19]}` to `{stats['end_time'][:19]}`  ",
        ...
        "#### 💰 Estimated Costs",
        "",
        "| Model | This Run | 100 runs/month |",
        "|-------|--------:|---------------:|",
        f"| GPT-4o-mini | ${stats['estimated_cost_gpt4o_mini']:.4f} | ${stats['estimated_cost_gpt4o_mini'] * 100:.2f}/mo |",
        f"| GPT-4o | ${stats['estimated_cost_gpt4o']:.4f} | ${stats['estimated_cost_gpt4o'] * 100:.2f}/mo |",
    ]
    return "\n".join(lines)
...
def main():
    ...
    parser.add_argument(
        "--tag", default="test", help="Tag to filter by (default: test)"
    )
    parser.add_argument(
        "--minutes", type=int, default=10, help="Look back N minutes (default: 10)"
    )
    parser.add_argument("--json", action="store_true", help="Output as JSON")
    parser.add_argument(
        "--markdown", action="store_true", help="Output as GitHub-flavored markdown"
    )
    ...
    try:
        ...
        if args.json:
            ...
            print(format_report(stats, markdown=args.markdown))
